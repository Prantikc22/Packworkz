  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #020817 0%, #071a45 40%, #0f2d7a 100%)", padding: "140px 40px 100px", textAlign: "center", minHeight: "70vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 60% 50% at 50% 50%, rgba(59,130,246,0.18) 0%, transparent 70%)" }} />
        <div style={{ position: "relative", zIndex: 1, maxWidth: 820, margin: "0 auto" }}>
          <div style={{ display: "inline-flex", alignItems: "center", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 9999, padding: "6px 18px", marginBottom: 28 }}>
            <span style={{ color: "#E8A838", fontSize: 10, fontWeight: 700, letterSpacing: "2.5px", textTransform: "uppercase" }}>PACKWORKZ TECHNOLOGY</span>
          </div>
          <h1 style={{ color: "white", fontSize: "clamp(2.8rem,6vw,5rem)", fontWeight: 900, lineHeight: 1.05, letterSpacing: "-2px", marginBottom: 28 }}>
            SmartStock™<br />
            <span style={{ color: "#60a5fa", fontStyle: "italic" }}>AI Inventory.</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.55)", fontSize: "clamp(16px,2vw,20px)", lineHeight: 1.7, maxWidth: 580, margin: "0 auto 48px" }}>
      ...
      <section style={{ background: "#08080f", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ maxWidth: 860, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", padding: "0 24px" }}>
          {[
            ...
            <div key={i} style={{ padding: "32px 16px", textAlign: "center", borderRight: i < 3 ? "1px solid rgba(255,255,255,0.06)" : "none" }}>
              ...
              <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "1.5px" }}>{s.label}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ color: "white", fontSize: "clamp(2rem,4vw,3rem)", fontWeight: 800, lineHeight: 1.1, letterSpacing: "-1px" }}>
              ...
              <span style={{ color: "rgba(255,255,255,0.5)", fontStyle: "italic" }}>than the packaging itself.</span>
            </h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16 }}>
            {[
              ...
              <div key={i} style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)", padding: "28px 24px" }}>
                <div style={{ width: 28, height: 2, background: "rgba(239,68,68,0.5)", marginBottom: 16 }} />
                <h3 style={{ color: "rgba(255,255,255,0.85)", fontSize: 15, fontWeight: 700, marginBottom: 10 }}>{p.title}</h3>
                <p style={{ color: "rgba(255,255,255,0.42)", fontSize: 13, lineHeight: 1.7 }}>{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── HOW SMARTSTOCK WORKS (high-level only) ── */}
            ...
            {[
              {
                ...
                desc: "SmartStock monitors your order velocity across seasons, campaigns, and market cycles — building a brand-specific demand model that improves with every order.",
                color: "#60a5fa",
              },
              {
                step: "02",
                title: "Predictive Reorder Triggers",
                desc: "Before your stock hits the danger zone, SmartStock raises a reorder flag — with the exact quantities, timing, and variant breakdown your production schedule needs.",
                color: "#a78bfa",
              },
              {
                step: "03",
                title: "Buffer Inventory Pre-positioning",
                desc: "For high-velocity SKUs, Packworkz pre-positions buffer stock at our fulfilment layer — enabling 5–7 day delivery vs. the industry standard 14–21 days.",
                color: "#34d399",
              },
              {
                step: "04",
                title: "Automatic SLA Protection",
                desc: "Every SmartStock order comes with 3 backup vendor assignments and a dispatch SLA guarantee. Your production line never waits.",
                color: "#f59e0b",
              },
            ].map((s, i) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: 24, padding: "32px 0", borderBottom: i < 3 ? "1px solid rgba(255,255,255,0.05)" : "none", alignItems: "flex-start" }}>
                <div style={{ textAlign: "center", paddingTop: 4 }}>
                  <span style={{ fontSize: "clamp(2.5rem,4vw,3.5rem)", fontWeight: 900, color: s.color, opacity: 0.25, lineHeight: 1, letterSpacing: "-2px", fontFamily: "'Space Grotesk', sans-serif" }}>{s.step}</span>
                </div>
                <div>
                  <h3 style={{ color: "white", fontSize: 18, fontWeight: 700, marginBottom: 10 }}>{s.title}</h3>
                  <p style={{ color: "rgba(255,255,255,0.48)", fontSize: 14, lineHeight: 1.75 }}>{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: "#0a0f1e", padding: "80px 40px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 15, lineHeight: 1.75, marginBottom: 48 }}>
            Brands with repeat orders across 2+ cycles are automatically evaluated for SmartStock eligibility. There's no form to fill, no integration to set up. Your account manager will inform you when you qualify.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 48 }}>
            {[
              ...
              <div key={i} style={{ background: "rgba(96,165,250,0.07)", border: "1px solid rgba(96,165,250,0.15)", padding: "20px 16px", textAlign: "center" }}>
                <p style={{ color: "white", fontSize: 18, fontWeight: 800, marginBottom: 6 }}>{c.val}</p>
                <p style={{ color: "rgba(255,255,255,0.40)", fontSize: 11, textTransform: "uppercase", letterSpacing: "1.5px" }}>{c.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative overflow-hidden" style={{ background: "#08080f", padding: "100px 40px", textAlign: "center" }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 50% 60% at 50% 100%, rgba(59,130,246,0.14) 0%, transparent 65%)" }} />
        <div style={{ position: "relative", zIndex: 1, maxWidth: 600, margin: "0 auto" }}>
          <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 11, fontWeight: 700, letterSpacing: "2.5px", textTransform: "uppercase", marginBottom: 16 }}>READY TO ELIMINATE STOCKOUTS</p>
