  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <section className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #020817 0%, #071a45 40%, #0f2455 100%)", padding: "140px 40px 100px", textAlign: "center", minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 60% 50% at 50% 40%, rgba(27,108,168,0.20) 0%, transparent 70%)" }} />
        <div style={{ position: "relative", zIndex: 1, maxWidth: 800, margin: "0 auto" }}>
          <div style={{ display: "inline-flex", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 9999, padding: "6px 18px", marginBottom: 28 }}>
            <span style={{ color: "rgba(255,255,255,0.50)", fontSize: 10, fontWeight: 700, letterSpacing: "2.5px", textTransform: "uppercase" }}>OUR MANUFACTURING BACKBONE</span>
          </div>
          <h1 style={{ color: "white", fontSize: "clamp(2.8rem,6vw,4.8rem)", fontWeight: 900, lineHeight: 1.05, letterSpacing: "-2px", marginBottom: 24 }}>
            500+ Verified<br />
            <span style={{ color: "#60a5fa", fontStyle: "italic" }}>Factory Partners.</span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.50)", fontSize: "clamp(15px,2vw,18px)", lineHeight: 1.75, maxWidth: 560, margin: "0 auto 48px" }}>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <Link href="/products">
              <button className="btn-fill btn-outline-white px-8 py-3 text-sm">Browse Products</button>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "#08080f", padding: "100px 40px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <p style={{ color: "rgba(255,255,255,0.42)", fontSize: 16, maxWidth: 500, margin: "16px auto 0", lineHeight: 1.7 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              ...
              { n: "04", title: "Compliance Review",    desc: "All certifications are verified, dated, and stored in our system. Expired certs trigger automatic quarantine of that factory from new orders." },
              { n: "05", title: "Performance Tracking", desc: "Every dispatch is scored on OTD, quality rejections, and packing accuracy. Low scorers are placed on review and eventually delisted." },
              ...
              <div key={i} style={{ background: "rgba(13,27,42,0.60)", border: "1px solid rgba(255,255,255,0.06)", borderLeft: "2px solid rgba(59,130,246,0.40)", padding: "24px 20px", display: "flex", gap: 16 }}>
                <span style={{ color: "rgba(255,255,255,0.18)", fontSize: 13, fontWeight: 800, fontFamily: "'Space Grotesk',sans-serif", flexShrink: 0, paddingTop: 2 }}>{s.n}</span>
                <div>
                  <h3 style={{ color: "rgba(255,255,255,0.88)", fontSize: 14, fontWeight: 700, marginBottom: 7 }}>{s.title}</h3>
                  <p style={{ color: "rgba(255,255,255,0.38)", fontSize: 13, lineHeight: 1.65 }}>{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: "#0a0f1e", padding: "100px 40px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
            {CATEGORIES.map((c, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", padding: "24px 20px" }}>
                ...
                <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 12, lineHeight: 1.6 }}>{c.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, maxWidth: 640, margin: "0 auto" }}>
            {CERTIFICATIONS.map((c, i) => (
              <div key={i} style={{ background: "rgba(96,165,250,0.06)", border: "1px solid rgba(96,165,250,0.15)", padding: "20px 16px", textAlign: "center" }}>
                <p style={{ color: "white", fontSize: 15, fontWeight: 800, marginBottom: 6 }}>{c.code}</p>
                <p style={{ color: "rgba(255,255,255,0.38)", fontSize: 11, textTransform: "uppercase", letterSpacing: "1.2px" }}>{c.name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative overflow-hidden" style={{ background: "#08080f", padding: "100px 40px", textAlign: "center", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 580, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <Link href="/products">
              <button className="btn-fill btn-outline-white px-8 py-3 text-sm">Browse 110+ SKUs</button>
            </Link>
          </div>
        </div>
      </section>
